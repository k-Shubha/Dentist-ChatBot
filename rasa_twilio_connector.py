...
class TwilioInput(InputChannel):
    ...
    def from_credentials(cls, credentials):
        ...
        print("sid",credentials.get("account_sid"))
        return cls(
    ...
    def blueprint(self, on_new_message):
        ...
        print("twilio webhook:",twilio_webhook)
        @twilio_webhook.route("/", methods=["GET"],defaults={'path': '/'})
        def health():
            print("ok")
            return jsonify({"status": "ok"})

        print("after webhook")
        @twilio_webhook.route("/webhook", methods=["GET","POST"])
